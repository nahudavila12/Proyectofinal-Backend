...
export function SearchBar({ filters, setFilters }: PropertyFiltersProps) {
  const [adults, setAdults] = useState(2);
  const [children, setChildren] = useState(0);
  const [rooms, setRooms] = useState(1);
  ...
  const validateInputs = () => {
    ...
    if (checkInDate < today) {
      return "La fecha de ingreso debe ser posterior a hoy.";
    }
    ...
    if (totalPeople > 50) {
      return "El máximo de personas es 50.";
    }
