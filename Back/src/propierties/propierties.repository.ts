...
import { ICategories, IRoomState, Room } from 'src/rooms/room.entity';
...
export class PropiertiesRepository {
  ...
  async addProperty() {
    ...
    for (const element of data) {
      ...
      if (
        Array.isArray(element.propertyImages) &&
        element.propertyImages.length > 0
      ) {
      ...
      if (Array.isArray(element.rooms) && element.rooms.length > 0) {
        for (const roomElement of element.rooms) {
          if (
            !roomElement.category ||
            !Object.values(ICategories).includes(
              roomElement.category as ICategories,
            )
          ) {
          ...
          if (
            Array.isArray(roomElement.roomImages) &&
            roomElement.roomImages.length > 0
          ) {
          ...
          if (
            ...
            Array.isArray(roomElement.services)
          ) {
  ...
  async findPropertiesByFilters(filters: PropertyFilters): Promise<Property[]> {
    ...
    if (filters.location) {
      queryBuilder.andWhere('property.location = :location', {
